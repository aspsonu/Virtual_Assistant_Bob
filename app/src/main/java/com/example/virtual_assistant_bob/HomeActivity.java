...
import static com.example.virtual_assistant_bob.Functions.fetchName;
import static com.example.virtual_assistant_bob.Functions.wishMe;
...
public class HomeActivity extends AppCompatActivity {
    ...
    private void response(String msg) {
        ...
        if (msgs.indexOf("bob")!=-1){
            ...
            speak("Sorry......, I am unable to recognize and process");
        }
        if (msgs.indexOf("call")!=-1){
            ...
            Dexter.withContext(this)
                ...
                @Override public void onPermissionsChecked(MultiplePermissionsReport multiplePermissionsReport) {
                    if (multiplePermissionsReport.areAllPermissionsGranted()){
                        if (checkForPreviousCallList(HomeActivity.this)) {
                            speak(makeCallFromSavedContactList(HomeActivity.this,name));
                        }
                        else{
                            HashMap<String, String> list = getContactList(HomeActivity.this,name);
                            if (list.size() > 1){
                                for (String i : list.keySet()){
                                    listName[0] = listName[0].concat("............................." + i);
                                }
                                speak("which one buddy ?.. There is " + listName[0]);
                            }
                            else if (list.size() == 1){
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                                    speak("calling." + name);
                                    delay(1200);
                                    makeCall(HomeActivity.this,list.values().stream().findFirst().get());
                                    clearContactListSavedData(HomeActivity.this);
                                }
                            }
                            else {
                ...
                @Override public void onPermissionRationaleShouldBeShown(List<PermissionRequest> list, PermissionToken permissionToken) {
